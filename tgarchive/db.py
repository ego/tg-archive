...
Chat = namedtuple(
    "Chat",
    [
        "id", "date", "archived", "is_channel", 
        "is_group", "is_user", "name", "title", "pinned", "full_dialog", "full_entity",
        "username"
    ]
)
...
Message = namedtuple(
    "Message",
    [
        "id", "type", "date", "edit_date", "content", 
        "reply_to", "user", "media", "full", "chat_id",
...
class DB:
    ...
    def insert_user(self, u: User):
        ...
        cur = self.conn.cursor()
        cur.execute("""INSERT INTO users (id, username, first_name, last_name, tags, avatar, full)
            VALUES(?, ?, ?, ?, ?, ?, ?) ON CONFLICT (id)
            DO UPDATE SET username=excluded.username, first_name=excluded.first_name,
                last_name=excluded.last_name, tags=excluded.tags, avatar=excluded.avatar, full=excluded.full
            """, (u.id, u.username, u.first_name, u.last_name, " ".join(u.tags), u.avatar, u.full))
    ...
    def _make_message(self, m) -> Message:
        ...
        id, typ, date, edit_date, content, reply_to, \
            ...
            media_id, media_type, media_url, media_title, media_description, \
            media_thumb, user_full, media_full, message_full, chat_id, \
            from_chat_id, chat_full_entity = m
